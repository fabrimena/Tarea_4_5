...
\textbf{Pregunta} & \textbf{Score del evaluador} \\
\midrule
Enfoque científico vs ingenieril  & 0.90 \\
...
\textbf{Pregunta} & \textbf{Score del evaluador} \\
\midrule
¿Cómo funciona un motor CVT?      & 0.70 \\
\$157 en colones (tipo de cambio) & 0.70 \\
¿Capital de Australia?            & 0.70 \\
Guitarra acústica vs eléctrica    & 0.50 \\
¿Receta del gallo pinto?          & 0.70 \\
\midrule
\textbf{Promedio}                 & \textbf{0.66} \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Pregunta} & \textbf{Score del evaluador} \\
\midrule
Última versión de LangChain       & 0.80 \\
Precio API GPT-4o                 & 0.90 \\
Novedades Google I/O 2025         & 0.70 \\
¿Qué es LangGraph?                & 0.70 \\
Modelos OpenAI lanzados en 2025   & 0.50 \\
\midrule
\textbf{Promedio}                 & \textbf{0.72} \\
\bottomrule
\end{tabular}
\end{table}

\begin{table}[htbp]
\caption{Scores del dataset \texttt{transactional} (Run txn-002, 5 ítems)}
...
\textbf{Pregunta} & \textbf{Score del evaluador} \\
\midrule
Resumen de riesgo MSR             & 0.70 \\
Transacción mayor monto           & 0.70 \\
Transacciones cliente 3           & 0.90 \\
Transacciones sospechosas CMV     & 0.90 \\
Transacciones dif. paises         & 0.70 \\
\midrule
\textbf{Promedio}                 & \textbf{0.78} \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Pregunta} & \textbf{Score del evaluador} \\
\midrule
Clasificación con 0.85            & 0.90 \\
Diferencia apr. supervisado       & 1.00 \\
Técnicas efectivas                & 0.50 \\
Comparación red neuronal          & 1.00 \\
Recomendación datasets grandes    & 1.00 \\
\midrule
...
En la tabla ~\ref{tab:resumen} se ve un resúmen del promedio de scores del evaluador LLM-as-judge. Se encuentra que en la mayoría de casos, los scores promedio del evaluador rondan por encima de 0.75, siendo la excepción el caso del dataset \texttt{fuera\_alcance}, el cual se otorgó menor nota por no responder la pregunta (comportamiento esperado del modelo), por lo que la menor nota es aceptable, mientras que el dataset \texttt{websearch} se redujo un poco de nota debido a que las respuestas esperadas no puede coincidir completamente con la búsqueda web. Se proporciona un archivo con los detalles de los experimentos de evaluación.
...
\midrule
\texttt{factual}        & 10 & 0.95 \\
\texttt{comparacion}    & 5  & 0.98 \\
\texttt{fuera\_alcance} & 5  & 0.66 \\
\texttt{websearch}      & 5  & 0.72 \\
\texttt{transactional}  & 5  & 0.78  \\
\texttt{conversacion}   & 5  & 0.88  \\
\bottomrule
\end{tabular}
\end{table}
...
La evaluación experimental sobre los datasets ejecutados demuestran una ejecución altamente aceptable del agente, utilizando sus herramientas a cómo sea necesario y obteniendo la información correspondientes.

\begin{thebibliography}{00}

\bibitem{evaluation_json}
JSON evaluacion modelo 2026 [Online]. Available: \url{https://github.com/fabrimena/Tarea_4_5/blob/bd98ef7a3330296294ea61809ecafc41627698e6/experimentos_export.json}.
